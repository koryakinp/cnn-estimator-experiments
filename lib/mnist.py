...
class MNIST:
    ...
    def _load_data(self, filename, offset):
        ...
        download(
          base_url=base_url, filename=filename, download_dir=self.data_dir)
    ...
    def get_next_train_batch(self, batch_size):
        ...
        start = self.train_idx * batch_size
        stop = self.train_idx * batch_size + batch_size

        if(stop > self.num_train):
            stop = self.num_train
    ...
    def get_number_of_train_batches(self, batch_size):
        return int(self.num_train/batch_size)

    def get_next_test_batch(self, batch_size):

        start = self.test_idx * batch_size
        stop = self.test_idx * batch_size + batch_size

        if(stop > self.num_test):
            stop = self.num_test
